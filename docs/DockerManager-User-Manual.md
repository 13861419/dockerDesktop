...
- **Log toolbar (new in 1.91.0)**: the log dialog now matches the container log dialog exactly — time-range filter (all / last 10 minutes / 1 hour / 4 hours / 24 hours), line-count options (100/200/500/1000/all), follow refresh (3s polling auto-scroll), timestamp toggle, clear view, copy, download (`<project>.log`), keyword highlight search (with hit count), wrap toggle and line numbers.
