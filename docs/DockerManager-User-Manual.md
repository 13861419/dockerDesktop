...
- **Details** — open the detail page; the log dialog supports **follow refresh** (SSE incremental push — lines arrive as they are produced, no more 3s polling full re-fetch; the wheel exits follow and freezes a snapshot, 1.92.0), **time-range filter** (all / last 10 minutes / 1 hour / 4 hours / 24 hours, filtered server-side), **timestamp toggle** (Docker timestamps prefixed per line), **line-count options** (100/200/500/1000/all), **clear view**, **inline search** (keyword highlight + hit count), **level filter chips** ("All / Errors / Warnings" single-select with hit counts, one click to focus on problem lines, 1.92.0), **copy all**, **wrap toggle**, line numbers and "jump to bottom", plus log file download. Maximize / restore lives in the dialog title bar for every dialog (unified since 1.91.0).
...
- **Environment / directory / service jump (new in 1.54.0)**: the project row gains an "Environment" entry (edit the project's `.env` in place — created on save; run "Start" again to apply) and a "Files" button that jumps straight to the file manager in the project directory. Service chips in the status column are clickable and open the matching container's detail page (terminal included).
- **State dropdown actions (new in 1.53.0)**: the project row's start / stop / restart buttons are consolidated into one state dropdown (same as the containers page) — the pill shows "running / stopped" by state and expands to pick an action, keeping the actions column tidy.
- **Editor fullscreen & version history (new in 1.52.0)**: the edit dialog can toggle fullscreen (about 40 visible lines, great for large files); the previous content is recorded on each save (last 20 kept per file). The "Version history" button lists entries with time and author — load any version into the editor and hit Save to roll back.
- **External project adoption (new in 1.51.0)**: Compose projects created outside the panel (manual `docker compose up` / third-party tools) are auto-discovered via container `com.docker.compose.project` labels, tagged "External" with running/total counts. External projects fully support editing the compose file (saved in place) and applying with `up -d`, plus all operations (start / stop / restart / pull / build / service actions / logs / stats / drift check). **Deleting an external project only removes containers and keeps the compose file**, so projects managed by tools like 1Panel stay intact.
- **Multi-select batch delete (new in 1.89.0)**: the project list gains a checkbox column with "select all"; the toolbar shows the selection count with one-click clear. "Batch delete" removes several projects at once (optionally deleting volumes too), running the same per-project teardown as single delete and reporting per-item success / failure — one failing project never aborts the batch. When the selection includes external (auto-adopted) projects, the confirmation dialog lists them and requires ticking "I understand external projects' containers will be stopped" before proceeding — external projects only have containers stopped, compose files are kept. API `POST /api/compose/batch-delete`.
- **External project fixes & privileged file access (new in 1.90.0)**: 16 APIs (config / structure / start-stop / logs / rolling update / drift fix, etc.) previously returned "project not found or missing compose file" for external projects; they now resolve projects via container labels. When the compose file is owned by root (e.g. the panel is co-hosted with 1Panel), the list still shows the project marked "file restricted" and reads / writes / start-stop run through the Docker helper container as host root. NDJSON output of `docker compose ps` with multiple containers parses correctly, so statuses no longer show as stopped.
- **Multi-file compose association & switcher (new in 1.90.0)**: projects started with `-f a.yml -f b.yml` list all related files ("main +N", hover to see them); start / stop / config commands carry the full `-f` set in label order, matching the running state exactly; the edit dialog can switch the target file (strict file allow-list on save). Each save snapshots the original file to the data directory `backups/compose-files/` (last 5 kept per project) as a second line of defence alongside version history.
- **Logs & status UX (new in 1.90.0)**: one-click fullscreen for the log dialog (log area fills the viewport, toggle back anytime); per-service "Logs" buttons in the structure view; ANSI color escapes are stripped from `docker compose logs`, so colored logs are no longer garbled; statuses load via a single batched API (`GET /api/compose/status`) making long lists noticeably faster; the More menu was reshuffled — "Edit" (the raw file) is now the primary entry and "Config" a secondary view of the normalized compose config.
- **Log toolbar (new in 1.91.0)**: the log dialog now matches the container log dialog exactly — time-range filter (all / last 10 minutes / 1 hour / 4 hours / 24 hours), follow refresh (3s polling auto-scroll), timestamp toggle, clear view, copy, download (`<project>.log`), keyword highlight search (with hit count), wrap toggle and line numbers. Line count adapts to the view: the last 200 lines normally, everything automatically when maximized.
- **Follow refresh via SSE live stream + level filter chips (new in 1.92.0)**: follow refresh has been upgraded from 3s polling to incremental SSE streaming (server-side `docker compose logs --follow`) — lines are pushed as they are produced, with no more multi-second delay or repeated full-tail fetches; changing the line count / time range / timestamp / service filters reconnects automatically and re-sends the tail history; the toolbar gains "All / Errors / Warnings" level filter chips (with hit counts); scrolling up with the wheel exits follow (the current stream content is frozen as a snapshot); external projects without compose file read access fall back to a hint suggesting manual refresh.
- **Container log follow switched to SSE, detail-page level filter, and site-wide security hardening (new in 1.92.0)**: the container log dialog's follow refresh now uses incremental SSE streaming as well (`GET /api/containers/:id/logs/stream` with new `since` / `timestamps` parameters); exiting follow freezes a snapshot; the container detail page's real-time log tab gains level filter chips (All / Errors / Warnings, with hit counts) plus error-red / warning-yellow line coloring; the server enables gzip compression site-wide (SSE excluded) and security response headers (`X-Frame-Options: DENY`, `nosniff`, `Referrer-Policy`, a baseline CSP); the operation audit log queries gain composite indexes for user / target-type filters.
...
- **Generate-Compose editor upgrades (1.47.0)**: the "Generate Compose" dialog (from containers) now uses a line-numbered, YAML-syntax-highlighted editor, with **fullscreen editing** and **AI review & optimization** — one click asks the AI to explain each service, flag issues and suggest actionable improvements.
...
9. **One-click domain binding (1.57.0, admins only)**: installed app cards gain a "Bind Domain" action — enter a domain (the host port is prefilled from the app's default mapping) and a reverse-proxy site is created and applied automatically, making the app reachable at `http://<domain>` right after deployment. If a matching ACME certificate exists on the "SSL Certificates" page, tick HTTPS to auto-fill the cert path and switch the listen port to 443 — closing the loop of deploy → bind domain → HTTPS.
10. **App export / sharing (1.60.0)**: every app card gains an "Export" action (admins) — a dialog shows the app's `apps.json`-compatible definition (built-in, custom, and Git-source apps all work), with one-click copy or download as `<app-id>.apps.json`; drop it into any git repo to re-import it via "App Sources", closing the app-sharing ecosystem loop. Install params are excluded by default; tick "Include install params" to attach port / env override values (review for sensitive info before sharing).
11. **Data-preserving upgrade (1.72.0)**: upgrading a Compose suite is fully automated — ① pre-upgrade snapshot: the panel records each service's current image ID and compose config (stored in `appstore-snapshots`); ② pull the new image and force-recreate containers (data volumes are untouched by recreation); ③ health-check window: two rounds × 8 s confirming all services are running; ④ automatic rollback on failure: if recreation fails or the health check does not pass, the old image is retagged back to its original tag → config restored → recreated at the pre-upgrade version, with data preserved throughout.
...
- **Root escalation (v1.34.1, auto-pull since 1.74.5)**: on Linux, when the panel service runs as a low-privileged user (default systemd `User=dockerman`), the terminal detects this and automatically enters a true host root shell through a **Docker helper container** (`--privileged --pid=host` + `nsenter` into the PID 1 namespaces; the connect banner names the helper image). When no helper image exists locally but the docker CLI is available, the panel automatically pulls alpine in the background (120 s timeout) and then enters the root shell — no manual `docker pull` needed. When the docker CLI is missing or the pull fails (offline / restricted network), it falls back to the service user with an explanatory banner. `sudo` cannot work under the service account (system account without a password, and systemd `NoNewPrivileges` blocks setuid); to get a native root shell instead, run the service as `User=root`. Since 1.75.4 the PTY inside the helper container is allocated by the host's `script` utility (docker keeps only `-i`), fixing the instant disconnect caused by the docker CLI refusing to pipe stdin into a `-t` container.
...
- **Container-level authorization (new in 1.38.0)**: the user management table has a "Container allowlist" column (CSV, `prefix*` wildcards supported, empty = unrestricted). Once configured, the user's container list only shows allowlisted containers; access to any other container's detail / start-stop / logs / files returns 403, and blocked attempts are written to the operation log. Admins are never restricted. **The terminal WebSocket is covered too (1.40.0)**: terminal connections to non-allowlisted containers are rejected and audited. **Aggregation surfaces filter visibility as well (1.41.0)**: global search / port map / network topology / log aggregation no longer reveal non-allowlisted containers, and pulling logs by container ID returns 403 when denied.
- **Admin password reset (new in 1.75.2)**: each row of the account table has a "Reset password" button (admins only; you cannot reset your own) — set a new password for any user without their old password. On reset, all of the user's active sessions are revoked immediately, the user is forced to change the password at next login, and the action is written to the audit log. The create-user and change-password forms use a compact single-row layout.
...
- **PWA & mobile**: the panel can be installed to a home screen (manifest + Service Worker app-shell cache, never intercepting /api and /ws); below 768px a mobile layout kicks in (collapsible sidebar, horizontally scrollable tables, touch-friendly controls). Since 1.34.0 the mobile layout adds a **bottom navigation bar** (Overview / Containers / Images / Logs / More) plus swipe gestures to open or close the sidebar drawer. **Since 1.87.0** the install flow is complete: Settings → About → "Install as app" — desktop Chrome / Edge show a native install prompt, iOS walks through Share → Add to Home Screen; once installed the panel opens full-screen in its own window; long-press the app icon for shortcuts to Containers / Images / Alerts / Settings; the Service Worker cache is versioned and the new worker takes over with an automatic refresh; `viewport-fit=cover` keeps the bottom nav clear of the iOS home indicator.
...
**Container self-heal**: configure automatic recovery rules per container name (exact match, stable across container recreation) or per Docker label (1.40.0).

| Field | Description |
| --- | --- |
| Container name | e.g. `nginx-proxy`; mutually exclusive with "Match label" |
| Match label (1.40.0) | Match a batch of containers by Docker label, e.g. `team=api` or just a key; takes precedence over the container name |
...
| Trigger limit (1.40.0) | Max triggers within the stats window (0 / empty = unlimited; window defaults to 3600s, adjustable 60–86400); when exceeded self-heal pauses and sends one danger alert |
| Scope (1.42.0) | Current engine only / All engines (incl. remote); with "all engines" every registered engine is checked, and events / notifications note the source engine |
...
**Self-heal execution records (1.39.0)**: every triggered self-heal action is archived (trigger / action / success / detail / time, last 200 kept). The "Recent Executions" table under the self-heal card shows the latest 50 and refreshes automatically after a manual sweep; filter by container name (fuzzy) and result (success / failure), and export CSV (1.42.0); API `GET /api/selfheal/events`, `GET /api/selfheal/events/export`.
...
The Engines page (`/engines`) toolbar gains "Batch Cleanup" (admin only): select multiple engines and a cleanup scope (stopped containers / dangling images / unused volumes / unused networks, multi-select), then run Docker prune in batch. An optional **age filter** limits the cleanup to objects unused for more than N hours (volume prune does not support the age filter — volumes are skipped automatically when an age filter is set). A **preview-only (dry-run) mode** lists the objects that would be deleted per engine before anything is removed. Running containers and in-use images are never touched; results are listed per engine (success / failure with cleaned counts), and the operation is written to the audit log; API `POST /api/engines/batch-prune`.
...
- **New in 1.76.2**: JWT decoder (pure frontend base64url, signature NOT verified, exp/iat shown as readable time), URL encode/decode, an HTTP status code reference with keyword filtering, and a byte unit converter (both 1024- and 1000-based).
...
**Linux upgrade self-healing (1.75.3)**: the deb / rpm upgrade script is hardened end to end — when package installation fails, it restarts the old-version service before reporting the failure (no more dead panel); after a successful install it waits for the old process to exit, then explicitly enables and starts the service with up to 3 retries; if the 60-second health check fails, it force-restarts once and allows another 30 seconds to recover from transient issues such as port-release races, and only then records the manual recovery command in the result file; the deb prerm no longer disables the service, so auto-start on boot survives a failed upgrade. Since 1.75.5 the script's cgroup escape is performed by the panel process itself (spawned via `systemd-run` with a timestamped unit name), so upgrades initiated from older panel versions benefit too; interrupted dpkg state is automatically recovered (`dpkg --configure -a`) before installing.

**Update reminders (1.74.0)**: the panel checks for new versions every 6 hours (first check 30 s after startup, results cached for 10 minutes). When a new version is found — ① a reminder is pushed via notification channels (following the alert routing, once per version); ② admins see a floating toast in the **bottom-right corner** of the UI (since 1.74.3; previously a top banner) with an "Update now" shortcut that jumps to the update-check row in Settings → About (smooth scroll + brief highlight, since 1.74.4). The toast can be dismissed and will not reappear for the same version. The system setting `update.checkEnabled` (on by default) controls the automatic check.
