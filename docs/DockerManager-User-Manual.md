...
**Update reminders (1.74.0)**: the panel checks for new versions every 6 hours (first check 30 s after startup, results cached for 10 minutes). When a new version is found — ① a reminder is pushed via notification channels (following the alert routing, once per version); ② admins see a banner at the top of the UI with an "Update now" shortcut to the one-click update. The banner can be dismissed and will not reappear for the same version. The system setting `update.checkEnabled` (on by default) controls the automatic check.
