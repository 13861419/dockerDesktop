...
- **Live resource graphs**: CPU, memory, and disk usage (ECharts). Since 1.28.10 the panel shows "host × container" dual dimensions: the CPU card leads with host-normalized usage (the Docker container figure remains as secondary and can exceed 100% on multi-core hosts); the CPU chart draws both "containers" and "host-wide" series, and the memory chart separates "host used" from "container total" (sum of all running containers' `mem_usage`, as a share of host memory). Host-level CPU alerts use the normalized host-wide figure.
