 ...
 *
 * 在宿主机（Windows）上维持一个长驻的 PowerShell / cmd 子进程，
...
/** 上调 shell 默认工作目录 */
const DEFAULT_CWD = process.env.USERPROFILE || process.env.HOME || 'C:\\';
...
/**
 * 为指定 shell 构造 UTF-8 输出前缀，避免中文 Windows 中文乱码
 * （与 REST 版保持一致）
 * @param shell shell 类型
 * @returns 编码前缀
 */
function withUtf8(shell: Shell): string {
  if (shell === 'cmd') return '';
...
function handleSession(ws: WebSocket): void {
  ...
  ws.on('message', (data) => {
    ...
    if (head.startsWith('CONFIG')) {
      ...
      const shell: Shell = buf.toString('utf8').split(',')[1] === 'cmd' ? 'cmd' : 'powershell';
      try {
        child = spawn(
          shell === 'powershell' ? 'powershell.exe' : 'cmd.exe',
          shell === 'powershell'
            ? ['-NoLogo', withUtf8(shell)]
            : [],
