...
async function main() {
  ...
  try {
    await login(page);
    await page.goto(`${BASE_URL}/cloudbackup`, { waitUntil: 'domcontentloaded' });
    await page.waitForTimeout(3000);
    const body = await page.locator('body').innerText();
    const shell = ['云端备份', '新增目标', '名称', '类型', '端点', '操作'];
    const m = shell.filter((x) => body.includes(x));
    console.log('PAGE /cloudbackup');
    console.log(`SHELL_MATCHED ${JSON.stringify(m)}`);
    console.log(`EMPTY ${body.includes('暂无云端目标')}`);
    ...
    if (fatal.length || body.includes('加载云端目标失败')) {
      throw new Error(`云备份页存在致命错误: ${fatal.join(' | ') || '加载云端目标失败'}`);
    }
  } finally {
    await browser.close();
  }
