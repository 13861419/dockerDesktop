/**
 * 镜像详情页 (/image/:name) 浏览器回归脚本
 * 先通过后端 API 取一个真实镜像名，再登录导航到详情页检查渲染，并收集网络/控制台错误。
 */
const fs = require('fs');
...
/** 获取第一个有名称的镜像（优先 RepoTags 非空） */
async function getFirstImageName(token) {
  ...
  const named = list.find((img) => Array.isArray(img.RepoTags) && img.RepoTags.length > 0 && img.RepoTags[0] !== '<none>:<none>');
  return named ? named.RepoTags[0] : (list[0].Id || null);
}
...
async function main() {
  ...
  const imageName = await getFirstImageName(token);
  ...
  try {
    await uiLogin(page);
    const url = imageName
      ? `${BASE_URL}/image/${encodeURIComponent(imageName)}`
      : `${BASE_URL}/image/nonexistent`;
    await page.goto(url, { waitUntil: 'domcontentloaded' });
    await page.waitForTimeout(3000);
    const body = await page.locator('body').innerText();
    const shell = ['使用该镜像的容器', '构建历史（History）', '导出', '层 ID', '命令'];
    const m = shell.filter((x) => body.includes(x));
    console.log(`PAGE /image/${imageName || 'nonexistent'}`);
    console.log(`IMAGE_DETECTED ${!!imageName}`);
    console.log(`SHELL_MATCHED ${JSON.stringify(m)}`);
    console.log(`MISSING ${body.includes('未找到镜像')}`);
    ...
    if (fatal.length || body.includes('拉取镜像详情失败')) {
      throw new Error(`镜像详情页存在致命错误: ${fatal.join(' | ') || '拉取镜像详情失败'}`);
    }
  } finally {
    await browser.close();
  }
