/**
 * 计划任务页 (/tasks) 浏览器回归脚本
 * 登录后导航到 /tasks，检查页面标题、列头与空状态是否正常渲染，并收集网络/控制台错误。
 */
const fs = require('fs');
...
async function main() {
  ...
  try {
    ...
    if (fatal.length || body.includes('拉取计划任务失败')) {
      throw new Error(`计划任务页存在致命错误: ${fatal.join(' | ') || '拉取计划任务失败'}`);
    }
  } finally {
    await browser.close();
  }
