...
async function main() {
  ...
  try {
    await login(page);
    await page.goto(`${BASE_URL}/engines`, { waitUntil: 'domcontentloaded' });
    await page.waitForTimeout(3000);
    const body = await page.locator('body').innerText();
    const shell = ['Docker 引擎', '新增引擎', '名称', '端点', '状态'];
    const m = shell.filter((x) => body.includes(x));
    console.log('PAGE /engines');
    console.log(`SHELL_MATCHED ${JSON.stringify(m)}`);
    console.log(`EMPTY ${body.includes('尚未配置引擎')}`);
    ...
    if (fatal.length || body.includes('加载引擎失败')) {
      throw new Error(`引擎页存在致命错误: ${fatal.join(' | ') || '加载引擎失败'}`);
    }
  } finally {
    await browser.close();
  }
