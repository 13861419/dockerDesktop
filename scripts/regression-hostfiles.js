/**
 * 宿主机文件页 (/hostfiles) 浏览器回归脚本
 * 登录后导航到 /hostfiles，检查页面标题、工具条按钮与列头是否正常渲染，并收集网络/控制台错误。
 */
const fs = require('fs');
...
async function main() {
  ...
  try {
    await login(page);
    await page.goto(`${BASE_URL}/hostfiles`, { waitUntil: 'domcontentloaded' });
    await page.waitForTimeout(3000);
    const body = await page.locator('body').innerText();
    const shell = ['宿主机文件', '上级', '新建文件夹', '新建文件', '上传', '名称', '修改时间', '操作'];
    const m = shell.filter((x) => body.includes(x));
    console.log('PAGE /hostfiles');
    console.log(`SHELL_MATCHED ${JSON.stringify(m)}`);
    console.log(`EMPTY ${body.includes('目录为空') || body.includes('未检测到磁盘')}`);
    ...
    if (fatal.length || body.includes('加载文件列表失败')) {
      throw new Error(`宿主机文件页存在致命错误: ${fatal.join(' | ') || '加载文件列表失败'}`);
    }
  } finally {
    await browser.close();
  }
