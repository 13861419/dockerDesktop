/**
 * 操作日志页 (/operation-logs) 浏览器回归脚本
 * 登录后导航到 /operation-logs，检查页面标题、列头与空状态是否正常渲染，并收集网络/控制台错误。
 */
const fs = require('fs');
...
async function main() {
  ...
  try {
    await login(page);
    await page.goto(`${BASE_URL}/operation-logs`, { waitUntil: 'domcontentloaded' });
    await page.waitForTimeout(3000);
    const body = await page.locator('body').innerText();
    const shell = ['操作日志', '操作时间', '操作人', '操作', '导出 CSV', '结果'];
    const m = shell.filter((x) => body.includes(x));
    console.log('PAGE /operation-logs');
    console.log(`SHELL_MATCHED ${JSON.stringify(m)}`);
    console.log(`EMPTY ${body.includes('暂无操作日志')}`);
    ...
    if (fatal.length || body.includes('加载操作日志失败')) {
      throw new Error(`操作日志页存在致命错误: ${fatal.join(' | ') || '加载操作日志失败'}`);
    }
  } finally {
    await browser.close();
  }
