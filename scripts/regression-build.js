/**
 * 构建镜像页 (/build) 浏览器回归脚本
 * 登录后导航到 /build，检查页面标题、表单字段、构建日志与构建历史区块是否正常渲染，并收集网络/控制台错误。
 */
const fs = require('fs');
...
async function main() {
  ...
  try {
    ...
    if (fatal.length) {
      throw new Error(`构建镜像页存在致命错误: ${fatal.join(' | ')}`);
    }
  } finally {
    await browser.close();
  }
