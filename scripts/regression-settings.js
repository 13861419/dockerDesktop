...
async function main() {
  ...
  try {
    ...
    if (fatal.length || body.includes('加载设置失败')) {
      throw new Error(`设置页存在致命错误: ${fatal.join(' | ') || '加载设置失败'}`);
    }
  } finally {
    await browser.close();
  }
