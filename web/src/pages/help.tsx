...
const QUICK_STEPS: Array<{ title: string; desc: string }> = [
  ...
  { title: '告警通知', desc: '「告警中心」配置 CPU / 内存 / 磁盘 / GPU / 容器级阈值规则，支持「连续周期」防毛刺与「多渠道路由」（全部启用渠道或按级别分流），渠道可自定义消息模板变量，通过 Webhook / 邮件 / 钉钉 / 飞书 / Telegram / 企业微信 / Slack 等渠道推送；「容器自愈」在健康检查失败或容器退出时自动重启/拉起（带冷却期防重）。' },
