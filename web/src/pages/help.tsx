...
const FEATURE_INDEX: Array<{ path: string; name: string; desc: string }> = [
  ...
  { path: '/compose', name: t('Compose'), desc: t('工程管理、yaml 编辑、docker run 命令导入、容器逆向推导；项目看板（服务级资源聚合）、服务滚动更新（健康检查+自动回滚）、全部滚动更新、漂移检测与一键修复（本地缺失可勾选删除，1.42.0）、跨引擎镜像分发与远端代理部署（1.34.0）') },
  ...
  { path: '/tasks', name: t('计划任务'), desc: t('定时备份/清理/跨引擎清理/构建/Webhook 触发') },
  ...
  { path: '/notifications', name: t('告警中心'), desc: t('告警规则（含连续周期防抖）、七类通知渠道、多渠道路由、送达率统计、磁盘写满与内存耗尽趋势预测、静默期值班渠道、容器自愈（标签匹配/触发上限/监控范围，执行记录筛选与导出，1.42.0）、记录与 AI 诊断') },
