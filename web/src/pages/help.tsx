...
const QUICK_STEPS: Array<{ title: string; desc: string }> = [
  ...
  { title: '告警通知', desc: '「告警中心」配置 CPU / 内存 / 磁盘 / GPU / 容器级阈值规则，支持「连续周期」防毛刺与「多渠道路由」（全部启用渠道或按级别分流），渠道可自定义消息模板变量，通过 Webhook / 邮件 / 钉钉 / 飞书 / Telegram / 企业微信 / Slack 等渠道推送；「送达率统计」按渠道汇总推送成功率与最近失败明细；「容器自愈」在健康检查失败或容器退出时自动重启/拉起（带冷却期防重）。' },
...
const FAQ_ITEMS: FaqItem[] = [
  ...
  {
    q: '支持哪些通知渠道？',
    a: '内置 Webhook、邮件（SMTP）、钉钉、飞书、Telegram、企业微信、Slack 七类渠道，可添加多个并按启停控制；「测试推送」可验证连通性，danger 级告警还可联动 AI 诊断。「推送路由」可选仅首个启用渠道、全部启用渠道或按 warn / danger / recovery 级别分流；每个渠道还可配置消息模板（{{level}} / {{message}} / {{time}} / {{channel}} 变量）自定义推送文案。',
  },
  {
    q: '如何给团队成员分配有限的操作权限？',
    a: '「设置 → 角色管理」新建自定义角色，按组勾选 14 项资源域权限（容器 / 镜像 / 卷 / 网络 / 编排 / 自愈），然后在「账号管理」中把用户角色设为该角色。角色成员只能在页面上看到已授权的操作按钮；容器终端等高危入口也按权限放行。角色权限仅作用于资源管理域，用户管理、系统设置、引擎切换等系统操作始终需要管理员。',
  },
  {
    q: 'AI 助手如何配置？',
  ...
  {
    q: '界面支持哪些语言？如何切换？',
    a: '「设置 → 关于 → 界面语言」可在中文 / English 间一键切换，偏好即时生效并记住（存于浏览器本地）。当前版本已覆盖导航、登录、公共组件与总览 / 容器 / 容器详情 / 通知中心 / 数据库 / 计划任务 / 应用商店 / Compose / 数据卷 / 审批中心 / 网络 / 引擎 / 镜像详情 / 备份 / 宿主机文件 / 设置等页面，其余页面暂以中文显示，后续版本持续补齐。',
  },
];
...
const FEATURE_INDEX: Array<{ path: string; name: string; desc: string }> = [
  ...
  { path: '/notifications', name: '告警中心', desc: '告警规则（含连续周期防抖）、七类通知渠道、多渠道路由、送达率统计、记录与 AI 诊断' },
  { path: '/events', name: '事件流', desc: 'Docker 事件实时流与持久化历史' },
  { path: '/tools', name: '工具箱', desc: 'JSON / 正则 / Base64 / 时间戳 / 进制 / 端口网段计算' },
  { path: '/approvals', name: '审批中心', desc: '高危操作审批与记录（含编排停止/批量删镜像/清理类）、近 30 天审批统计、记录导出 CSV' },
