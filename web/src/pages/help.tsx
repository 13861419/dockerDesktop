...
const FAQ_ITEMS: FaqItem[] = [
  ...
  {
    q: t('外部项目的 compose 文件提示「文件受限」还能编辑吗？'),
    a: t('能（1.90.0）：编排文件属主为 root（如面板与 1Panel 同机）时项目照常显示并标注「文件受限」，编辑与启停操作会经 Docker 助手容器以宿主机 root 提权执行；每次保存前原文件自动快照到数据目录 backups/compose-files/（每项目保留 5 份），写坏可手动还原。多文件（-f）启动的项目在编辑弹窗顶部可切换目标文件。'),
  },
  {
  ...
  {
    q: t('容器日志太多，如何快速定位报错？'),
    a: t('容器日志弹窗与 Compose 日志弹窗（1.92.0）均提供「全部 / 错误 / 警告」级别筛选 chips（附命中计数），一键只看报错行；可叠加关键字搜索高亮与时间范围过滤。两者（含容器详情页实时日志）的「跟随刷新」均为 SSE 实时流，日志产生即推送，开启后自动滚底，滚轮向上滚动即退出跟随并固化快照。'),
  },
  {
    q: t('误删了容器怎么办？'),
    a: t('容器回收站（1.92.0）帮你找回：经面板删除的容器（单个 / 批量 / 审批通过后）都会自动保存完整配置快照（保留最近 100 条），在容器页工具栏「回收站」中可按原配置一键重建，支持改名规避同名冲突；注意经 docker compose down 等面板外途径删除的容器不在回收站范围内。'),
  },
  {
    q: t('首装向导会再次出现吗？'),
...
const FEATURE_INDEX: Array<{ path: string; name: string; desc: string }> = [
  ...
  { path: '/containers', name: t('容器'), desc: t('生命周期管理、终端、日志、标签过滤、批量操作、镜像自动更新与回滚、Inspect 查看、容器内进程与操作记录、配置快照对比（1.50.0）；执行命令弹窗（1.92.0）：容器内运行单条命令并查看输出与退出码；日志弹窗（1.91.0）：时间范围过滤、时间戳、跟随刷新、条数（100/200/500/1000/全部）、清空、搜索高亮、下载；级别筛选 chips（1.92.0）：全部 / 错误 / 警告一键聚焦问题行，跟随刷新为 SSE 实时流；容器回收站（1.92.0）：面板删除的容器自动存配置快照，可按原配置一键重建') },
  ...
  { path: '/compose', name: t('Compose'), desc: t('工程管理、yaml 编辑、docker run 命令导入、容器逆向推导；外部项目自动纳管（1.51.0，可编辑第三方项目 compose 文件）；多选批量删除（1.89.0，含外部项目强确认）；外部项目 16 接口统一解析与 root 提权读写、多文件编排关联与切换编辑、保存前物理备份、日志一键放大与服务级日志（1.90.0）；日志弹窗工具栏对齐容器页（过滤/跟随刷新/时间戳/清空/搜索/下载，放大即看全部，1.91.0）；跟随刷新为 SSE 实时流、级别筛选 chips（1.92.0）；编辑器可放大、版本历史回退（1.52.0）、启停重启状态下拉（1.53.0）、环境变量 .env 编辑与目录直达（1.54.0）；项目看板（服务级资源聚合）、服务滚动更新（健康检查+自动回滚）、全部滚动更新、漂移检测与一键修复（本地缺失可勾选删除，1.42.0）；漂移比对维度含卷挂载/网络/healthcheck/labels（1.43.0）、跨引擎镜像分发与远端代理部署（1.34.0）') },
