...
const FEATURE_INDEX: Array<{ path: string; name: string; desc: string }> = [
  ...
  { path: '/compose', name: t('Compose'), desc: t('工程管理、yaml 编辑、docker run 命令导入、容器逆向推导；外部项目自动纳管（1.51.0，可编辑第三方项目 compose 文件）；编辑器可放大、版本历史回退（1.52.0）、启停重启状态下拉（1.53.0）、环境变量 .env 编辑与目录直达（1.54.0）；项目看板（服务级资源聚合）、服务滚动更新（健康检查+自动回滚）、全部滚动更新、漂移检测与一键修复（本地缺失可勾选删除，1.42.0）；漂移比对维度含卷挂载/网络/healthcheck/labels（1.43.0）、跨引擎镜像分发与远端代理部署（1.34.0）') },
