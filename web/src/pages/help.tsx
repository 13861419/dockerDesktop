...
const FEATURE_INDEX: Array<{ path: string; name: string; desc: string }> = [
  ...
  { path: '/settings', name: t('设置'), desc: t('账号、角色管理（RBAC）、2FA 两步验证、在线会话、IP 白名单、密码策略、容器白名单（资源级授权）、MCP 接入、界面语言切换、备份、AI 配置、系统参数、用户管理、设置变更审计（旧值→新值，1.44.0）；一键更新：自动检测安装类型，检查到新版本后免下载原地升级（1.69.0），升级前自动备份、失败自动回滚（1.73.0）；每 6 小时自动检查新版本，通知渠道推送 + 界面横幅提醒（1.74.0）') },
