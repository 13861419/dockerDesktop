...
const QUICK_STEPS: Array<{ title: string; desc: string }> = [
  ...
  { title: t('编排部署'), desc: t('「Compose」编写或由容器逆向生成 yaml（支持行号高亮编辑器、全屏与 AI 审查优化，1.47.0）；「应用商店」一键部署常用应用；「编排」管理启动依赖顺序。') },
  { title: t('告警通知'), desc: t('「告警中心」配置 CPU / 内存 / 磁盘 / GPU / 容器资源异常 / 容器级阈值规则，支持「连续周期」防毛刺与「多渠道路由」（全部启用渠道或按级别分流），渠道可自定义消息模板变量，通过 Webhook / 邮件 / 钉钉 / 飞书 / Telegram / 企业微信 / Slack 等渠道推送；「送达率统计」按渠道汇总推送成功率与最近失败明细；「容器自愈」在健康检查失败或容器退出时自动重启/拉起（带冷却期防重）。') },
...
const FAQ_ITEMS: FaqItem[] = [
  ...
  },
  {
    ...
    a: t('使用「Edge 节点」：面板创建节点拿到一次性 token，在远程主机上运行零依赖 agent（PANEL_URL / EDGE_TOKEN 两个环境变量），agent 主动反向连接面板，即可在面板中直接查看远端容器、镜像、网络与卷（1.63.0）。agent 版本落后时可在节点「更多▾」菜单一键升级（1.71.0）；「资源」按钮可查看远端主机 CPU / 内存曲线。'),
  },
  {
  ...
  {
    q: t('应用商店升级应用会丢数据吗？'),
    a: t('不会（1.72.0）：Compose 套件升级前面板自动记录各服务镜像与配置快照，升级后健康检查窗口确认全部服务正常运行；若新版本起不来，自动回滚到升级前版本（旧镜像重新标记 + 配置还原 + 重建），数据卷全程不受影响。'),
  },
  {
  ...
  {
    q: t('如何查看 30 天 / 90 天的历史资源曲线？'),
    a: t('总览「资源监控」与容器详情「资源曲线」的时间窗支持 10 分钟 / 1 小时 / 24 小时 / 7 天 / 30 天 / 90 天。7 天内为原始采样（30 秒粒度），30/90 天来自小时级聚合；原始与聚合保留天数、告警记录保留条数均可在「设置 → 系统参数 → 数据保留」调整，可观察周级 / 月级容量与负载趋势。NVIDIA 主机的 GPU 利用率趋势曲线共用同一时间窗。总览内存 / 磁盘曲线在趋势上升时还会绘制红色「趋势外推」虚线，直观预判写满 / 耗尽时点。'),
  },
  {
    q: t('如何让面板定时做漏洞扫描并在出现新漏洞时提醒？'),
    a: t('「计划任务」新建类型为「漏洞定时扫描」的任务：镜像列表可留空（自动扫描本地镜像前 N 个），建议按周配置 cron 并勾选「新增高危时推送告警」。每次扫描与上次按 CVE id 差集对比，新增 Critical / High 即推送到全部启用渠道；历史结果在镜像详情页「漏洞扫描」卡片中查看。'),
  },
  {
    q: t('审批流支持多级签批吗？'),
    a: t('系统参数「安全」组的「两级审批动作」填入动作类型（如 container.delete），该动作的审批单需两级签批：第一级运维或管理员、末级必须管理员；审批中心会显示「第 1/2 级」进度与 AP- 单号，每次签批均留痕并推送通知。待审批超过 TTL 的 3/4 时自动催办一次。'),
  },
  {
  ...
  {
    q: t('批量删除 Compose 项目会影响外部项目吗？'),
    a: t('批量删除支持一次删除多个项目（可同时删除数据卷），逐项执行并反馈结果。若勾选中含「外部」项目（自动纳管的第三方编排），确认弹窗会列出项目名并要求显式勾选确认——外部项目只会下线容器，compose 文件保留，重新 up 即可恢复运行。'),
  },
  {
  ...
  {
    q: t('计划任务如何实现「A 成功后自动执行 B」？'),
    a: t('编辑任务 A，在「链式触发（成功后执行）」下拉中选择任务 B 即可（1.84.0）。保存时自动校验下游存在性并拒绝成环（自环 / 间接环），执行侧另有最多 10 层深度兜底；定时与手动 / Webhook 执行同样级联，下游停用或执行中被占用时静默跳过。适合「备份 → 清理 → 部署」这类串行流水线。'),
  },
  {
    q: t('同一任务想换参数重复执行怎么办？'),
    a: t('任务配置中可写 {{占位符}}（如命令 docker stop {{TARGET}}），并在任务编辑里保存「默认运行参数（JSON）」（1.84.0）。点「立即执行」时会弹出参数收集框，按需填写后执行；定时 / Webhook 执行则自动套用默认参数。占位符按合法标识符（字母数字下划线）解析，未提供值的原样保留。'),
  },
  {
    q: t('首装向导会再次出现吗？'),
    a: t('不会自动出现：首装向导只在全新安装后首次以 admin 登录时弹出一次，存量数据库升级不会弹出；完成后可在「设置 → 关于 → 重看首装向导」随时再次打开。'),
  },
];
...
const FEATURE_INDEX: Array<{ path: string; name: string; desc: string }> = [
  ...
  { path: '/compose', name: t('Compose'), desc: t('工程管理、yaml 编辑、docker run 命令导入、容器逆向推导；外部项目自动纳管（1.51.0，可编辑第三方项目 compose 文件）；多选批量删除（1.89.0，含外部项目强确认）；编辑器可放大、版本历史回退（1.52.0）、启停重启状态下拉（1.53.0）、环境变量 .env 编辑与目录直达（1.54.0）；项目看板（服务级资源聚合）、服务滚动更新（健康检查+自动回滚）、全部滚动更新、漂移检测与一键修复（本地缺失可勾选删除，1.42.0）；漂移比对维度含卷挂载/网络/healthcheck/labels（1.43.0）、跨引擎镜像分发与远端代理部署（1.34.0）') },
  { path: '/deploys', name: t('Git 部署'), desc: t('Git 仓库绑定为部署应用，一键 compose up --build；Webhook 自动部署、HMAC 签名校验（1.46.0）、加密凭据、部署历史与失败告警；CI 状态门禁——先查外部 CI 绿了才部署，失败策略可选，绿快照一键回滚（1.75.0）；GitOps 定时同步——轮询分支新提交自动部署或提醒（1.77.0）；部署前后钩子（1.78.0）；CI 运行记录只读看板（1.79.0）；镜像构建推送——pull→build→push→up 完整流水线（1.80.0）；buildx 多架构构建 amd64+arm64（1.83.0）；凭据库集中管理与一键复制应用（1.85.0）') },
  { path: '/appstore', name: t('应用商店'), desc: t('一键部署常用应用，升级前版本对比（1.46.0）、安装前端口冲突预检（1.49.0）、Git 应用源批量引入（1.55.0）、一键绑定域名与 HTTPS 闭环（1.57.0）、应用导出分享（1.60.0）') },
  { path: '/tasks', name: t('计划任务'), desc: t('定时备份/清理/跨引擎清理/构建/Webhook 触发，执行历史按保留天数自动清理（1.43.0）；失败记录一键重跑（1.49.0）；备份成功自动上传云端目标（1.50.0）；任务级超时控制、失败自动重试与通知分级（1.81.0）；链式编排（成功后触发下游任务，防环检测）与带参执行（{{占位符}} 替换，1.84.0）') },
  ...
  { path: '/settings', name: t('设置'), desc: t('账号、角色管理（RBAC）、2FA 两步验证、在线会话、IP 白名单、密码策略、容器白名单（资源级授权）、管理员重置用户密码（1.75.2）、MCP 接入、界面语言切换、备份、AI 配置、系统参数、用户管理、设置变更审计（旧值→新值，1.44.0）；一键更新：自动检测安装类型，检查到新版本后免下载原地升级（1.69.0），升级前自动备份、失败自动回滚（1.73.0）；特权辅助单元 + 权限预检，无权限时保持在线并给出手动命令（1.82.0）；每 6 小时自动检查新版本，通知渠道推送 + 界面右下角浮层提醒（1.74.0）；安装到桌面（PWA）——原生安装确认 / iOS 引导，长按图标快捷方式直达高频页面（1.87.0）；重看首装向导——欢迎改密提醒、环境一键检测与竞品迁移对照随时回看（1.88.0）') },
  { path: '/logs', name: t('日志聚合'), desc: t('跨容器实时聚合与历史检索（持久化索引可配置保留天数）、FTS5 全文检索与命中分布（1.58.0）、导出') },
  ...
  { path: '/tools', name: t('工具箱'), desc: t('JSON / 正则 / Base64 / 时间戳 / 进制 / 端口网段计算 / YAML 校验格式化 / Cron 表达式解析 / DNS 解析查询 / SSL 证书查看器 / Hash 校验 / 密码 UUID 生成 / 文本 Diff 对比 / JWT 解码 / URL 编解码 / HTTP 状态码 / 字节单位换算') },
