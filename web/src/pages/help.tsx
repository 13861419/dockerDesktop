...
const FEATURE_INDEX: Array<{ path: string; name: string; desc: string }> = [
  ...
  { path: '/compose', name: t('Compose'), desc: t('工程管理、yaml 编辑、docker run 命令导入、容器逆向推导；项目看板（服务级资源聚合）、服务滚动更新（健康检查+自动回滚）、全部滚动更新与漂移检测（1.38.0）、跨引擎镜像分发与远端代理部署（1.34.0）') },
  ...
  { path: '/engines', name: t('Docker 引擎'), desc: t('多引擎管理与切换；跨引擎批量清理（批量 prune 停止容器/悬空镜像，1.39.0）；内置 pull-through 镜像拉取缓存（registry:2，端口 5060，1.34.0）') },
  ...
  { path: '/settings', name: t('设置'), desc: t('账号、角色管理（RBAC）、2FA 两步验证、在线会话、IP 白名单、密码策略、容器白名单（资源级授权）、MCP 接入、界面语言切换、备份、AI 配置、系统参数、用户管理') },
  { path: '/logs', name: t('日志聚合'), desc: t('跨容器实时聚合与历史检索（持久化索引可配置保留天数）、导出') },
  { path: '/operation-logs', name: t('操作日志'), desc: t('全量操作审计') },
  { path: '/notifications', name: t('告警中心'), desc: t('告警规则（含连续周期防抖）、七类通知渠道、多渠道路由、送达率统计、磁盘写满与内存耗尽趋势预测、静默期值班渠道、容器自愈与执行记录留档（1.39.0）、记录与 AI 诊断') },
