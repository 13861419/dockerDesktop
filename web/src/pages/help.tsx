...
const FEATURE_INDEX: Array<{ path: string; name: string; desc: string }> = [
  ...
  { path: '/appstore', name: t('应用商店'), desc: t('一键部署常用应用，升级前版本对比（1.46.0）') },
  { path: '/tasks', name: t('计划任务'), desc: t('定时备份/清理/跨引擎清理/构建/Webhook 触发，执行历史按保留天数自动清理（1.43.0）') },
  ...
  { path: '/backups', name: t('备份恢复'), desc: t('数据卷 / Compose / 站点备份；支持云端上传与从云端拉回恢复（1.43.0）') },
  { path: '/databases', name: t('数据库'), desc: t('MySQL / PostgreSQL / Redis 可视化，表数据一键导出 CSV（1.46.0）') },
  ...
  { path: '/notifications', name: t('告警中心'), desc: t('告警规则（含连续周期防抖）、七类通知渠道、多渠道路由、送达率统计、磁盘写满与内存耗尽趋势预测、静默期值班渠道、容器自愈（标签匹配/触发上限/监控范围，执行记录筛选与导出，1.42.0）、未保存渠道直接测试连接（1.43.0）、站点证书到期提醒（1.44.0）、记录与 AI 诊断') },
