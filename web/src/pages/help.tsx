...
const FEATURE_INDEX: Array<{ path: string; name: string; desc: string }> = [
  ...
  { path: '/deploys', name: t('Git 部署'), desc: t('Git 仓库绑定为部署应用，一键 compose up --build；Webhook 自动部署、HMAC 签名校验（1.46.0）、加密凭据、部署历史与失败告警；CI 状态门禁——先查外部 CI 绿了才部署，失败策略可选，绿快照一键回滚（1.75.0）；GitOps 定时同步——轮询分支新提交自动部署或提醒（1.77.0）；部署前后钩子（1.78.0）；CI 运行记录只读看板（1.79.0）；镜像构建推送——pull→build→push→up 完整流水线（1.80.0）') },
