...
const FAQ_ITEMS: FaqItem[] = [
  ...
  },
  {
    ...
    a: t('总览「资源监控」与容器详情「资源曲线」的时间窗支持 10 分钟 / 1 小时 / 24 小时 / 7 天 / 30 天 / 90 天。7 天内为原始采样（30 秒粒度），30/90 天来自小时级聚合（自动保留 90 天），可观察周级 / 月级容量与负载趋势。NVIDIA 主机的 GPU 利用率趋势曲线共用同一时间窗。'),
  },
  {
...
const FEATURE_INDEX: Array<{ path: string; name: string; desc: string }> = [
  ...
  { path: '/images', name: t('镜像'), desc: t('列表、搜索、拉取（多源容灾）、导出、层分析、漏洞扫描、镜像信任锁定（sha256 摘要漂移检测，1.34.0）') },
  ...
  { path: '/compose', name: t('Compose'), desc: t('工程管理、yaml 编辑、docker run 命令导入、容器逆向推导；项目看板（服务级资源聚合）、服务滚动更新（健康检查+自动回滚）、跨引擎镜像分发与远端代理部署（1.34.0）') },
  ...
  { path: '/engines', name: t('Docker 引擎'), desc: t('多引擎管理与切换；内置 pull-through 镜像拉取缓存（registry:2，端口 5060，1.34.0）') },
  ...
  { path: '/notifications', name: t('告警中心'), desc: t('告警规则（含连续周期防抖）、七类通知渠道、多渠道路由、送达率统计、磁盘写满与内存耗尽趋势预测、静默期值班渠道、记录与 AI 诊断') },
