 ...
 * 可复用的容器内文件浏览器组件
 *
 * 从独立的文件管理页（pages/files.tsx）抽取核心浏览逻辑，支持在容器详情页等
 * 任意已知 containerId 的场景下直接嵌入使用，免去跳转到独立文件管理页。
 *
 * 能力：目录浏览（面包屑导航）、文件预览、下载、上传、新建目录、重命名、删除。
 * 写操作（上传/新建目录/重命名/删除）受 useCanManage 权限管控，仅管理员可用，
 * 普通用户可只读浏览与下载。
 *
 * 后端接口（routes/files.ts，均以 /api/files/:containerId 为前缀）：
 *  - GET  /ls?path=         列目录
 ...
 * 容器内文件浏览器组件
 *
 * 用法：在已知容器 id 的页面中直接嵌入
 *   <FileExplorer containerId={id} />
 */
export default function FileExplorer({ containerId, className }: FileExplorerProps) {
  ...
  const handlePreview = async (item: ContainerFileItem) => {
    ...
    setPreview(null);
    try {
      const data = await get<PreviewData>(`/api/files/${containerId}/read`, { path });
  ...
  const handleDownload = async (item: ContainerFileItem) => {
    ...
    try {
      await download(
        `/api/files/${containerId}/download?path=${encodeURIComponent(path)}`,
        item.name,
      );
      showToast('已开始下载');
    } catch (e: any) {
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      ...
      const resp = await fetch(
        `/api/files/${containerId}/upload?path=${encodeURIComponent(currentPath())}&name=${encodeURIComponent(
          file.name,
        )}`,
  ...
  const handleMkdir = async () => {
    ...
    setCreatingDir(true);
    try {
      await post(`/api/files/${containerId}/mkdir`, {
  ...
  const handleRename = async () => {
    ...
    setRenaming(true);
    try {
      await post(`/api/files/${containerId}/rename`, {
  ...
  const handleDelete = async () => {
    if (!containerId || !deleteTarget) return;
    if (!canManage || checking) {
    ...
    setDeleting(true);
    try {
      await post(`/api/files/${containerId}/delete`, {
