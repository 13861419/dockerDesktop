...
- **容器管理**：列表、启停/删除/重启/克隆、镜像过滤、查看日志与详情、内置 Web 终端（xterm.js + WebSocket）、设置重启策略等；详情页含 Inspect 原始 JSON 查看（复制/下载）、容器内进程表（docker top）、**运行期文件变更查看（docker diff，1.48.0）**、按容器的操作记录留痕；**镜像自动更新**（按计划任务周期拉取镜像，有更新按原配置重建容器，健康检查未通过自动回滚，信任锁定摘要不符自动拦截）；**日志工具栏（1.91.0）**——时间范围过滤 / 时间戳开关 / 跟随刷新 / 条数档位（100/200/500/1000/全部）/ 清空 / 搜索高亮 / 下载
