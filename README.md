...
- **容器管理**：列表、启停/删除/重启/克隆、镜像过滤、查看日志与详情、内置 Web 终端（xterm.js + WebSocket）、设置重启策略等；详情页含 Inspect 原始 JSON 查看（复制/下载）、容器内进程表（docker top）、**运行期文件变更查看（docker diff，1.48.0）**、按容器的操作记录留痕；**镜像自动更新**（按计划任务周期拉取镜像，有更新按原配置重建容器，健康检查未通过自动回滚，信任锁定摘要不符自动拦截）；**日志工具栏（1.91.0）**——时间范围过滤 / 时间戳开关 / 跟随刷新 / 条数档位（100/200/500/1000/全部）/ 清空 / 搜索高亮 / 下载；**级别筛选 chips（1.92.0）**——全部 / 错误 / 警告一键聚焦问题行（附命中计数），跟随刷新为 SSE 实时流（日志产生即推送）；**容器回收站（1.92.0）**——面板删除的容器自动保存配置快照（保留最近 100 条），可按原配置一键重建（可改名）
...
- **数据卷 / 存储 / 网络**：卷与网络列表、创建、删除、清理未使用；存储使用统计与一键回收；**网络连通性诊断**（源容器 → 目标容器的 DNS 解析 / 容器间 TCP / 宿主机端口三项检查，1.48.0）
- **Compose**：编排文件查看与编辑、`docker compose up / down / pull / build`、**docker run 命令一键导入**（粘贴 `docker run` 自动转换为 compose YAML，端口 / 卷 / 环境变量 / 重启策略 / 资源限制 / 能力位全映射，不支持的选项逐条提示）；生成的 Compose 用**行号 + 语法高亮编辑器**编辑，支持**全屏编辑**与 **AI 审查优化**（1.47.0）
- **应用商店（AppStore）**：内置应用目录，一键安装部署；Compose 套件升级前显示**当前版本 → 最新版本**对比（可升级徽标，1.46.0）；**保留数据升级**——升级前自动快照（镜像 ID + 配置），升级后健康检查未通过自动回滚，数据卷全程不受影响（1.72.0）；安装前**端口冲突预检**（冲突直接拦截并列明占用来源，1.49.0）；支持 **Git 应用源**——任意 git 仓库放置 `apps.json` 清单即可批量引入应用（1.55.0）；应用可**一键导出**为 apps.json 分享片段（1.60.0）
...
- **Edge 节点（1.63.0 ~ 1.65.0）**：远程主机运行零依赖 agent 反向连接面板（NAT / 防火墙友好，支持 curl | sh 一键安装），面板内直接管理远端 Docker——容器全生命周期（启停 / 重启 / 删除 / 部署 / 日志）+ 远端事件并入统一事件流与自动化（跨节点自愈）；**agent 自动更新**（版本落后自动覆盖重启，`EDGE_AUTO_UPGRADE=0` 可关；`EDGE_RESTART=spawn` 兼容无 systemd 主机；「全部升级」批量刷新，1.86.0）与**节点资源监控**（CPU / 内存使用率曲线，1.71.0）
...
- **操作日志**：管理操作审计日志，含统计与 JSON / CSV 导出；**保留天数可配置**（默认 90 天），超期数据每日自动清理，AI 用量明细 / 巡检记录同模式（默认 30 天），计划任务执行历史保留同模式（默认 90 天，1.43.0）
- **告警中心 / 通知渠道（C3）**：宿主级（CPU / 内存 / 磁盘 / GPU / 网络带宽 Mbps / 容器资源异常）与容器级（退出 / 健康状态 / 端口监听 / CPU 内存阈值）告警规则；支持**静默时段 / 仅工作日 / 工作时间**；**推送窗口聚合防风暴**（窗口内多条告警合并为一条摘要，恢复通知始终即时，不同级别分桶发送）；**连续周期防抖**（连续 N 个采样周期超阈值才触发，过滤瞬时毛刺）；**多渠道路由**（仅首个 / 全部启用渠道 / 按级别路由：warn / danger / recovery 各自勾选目标渠道）；**渠道消息模板**（{{level}} / {{message}} / {{time}} / {{channel}} 变量按渠道渲染）；**容器自愈**（按容器名监听健康检查失败或退出，自动重启/拉起，冷却期防重，全程留痕推送，**执行记录留档**可在页面查看最近 200 次触发明细，支持**按 Docker 标签匹配**一批容器与**窗口内触发次数上限**（防崩溃循环无限重启，1.40.0），**监控范围可选仅当前引擎或全部引擎**（含远端，1.42.0），执行记录支持**按容器名/结果筛选并导出 CSV**（1.42.0））；通知渠道（Webhook / 邮件 / 钉钉 / 飞书 / Telegram / 企业微信 / Slack，敏感凭证加密存储）；**渠道送达率统计**（按渠道聚合推送成功 / 失败 / 成功率与最近失败明细）；**高危操作审批流**（非管理员危险操作转审批，审批中心含近 30 天统计看板与**记录导出 CSV**）；告警记录查询、**CSV 导出 / 归档**、实时资源快照手工检测；**磁盘应急自动清理**（写满预测触发联动镜像 GC，开关 + 24h 冷却期 + 留痕推送，1.49.0）
...
- **跨引擎聚合总览**：一次展示多台 Docker 主机的资源与对象数量（单引擎离线不影响其它）；**跨引擎批量清理**（勾选多台引擎批量 prune 停止容器 / 悬空镜像 / 未使用卷 / 未使用网络，支持按年龄过滤与 dry-run 预览，逐引擎汇总结果，仅管理员；可建**跨引擎清理计划任务**按周期自动执行，1.42.0）
...
- **Webhook / Git 自动部署**：计划任务支持匿名 **Webhook token** 触发（`POST /api/webhook/:token`，可选 Header 二次校验）与 **Git 仓库 clone / pull 自动部署**（HTTPS / SSH 凭证加密存储）；Git 部署 Webhook 支持 **HMAC 签名校验**（X-Hub-Signature-256，GitHub / Gitea 兼容，1.46.0）
- **用户与鉴权**：登录鉴权（连续失败锁定并**持久化**，另含同 IP 滑动窗口限制防换用户名绕过，1.43.0）、登录行为审计（成功 / 失败 / 被拒含 IP 与 UA，1.44.0）、**登录提醒**（成功 / 失败推送到通知渠道，附账号 / IP / 时间，1.49.0）、会话管理、用户增删/改密、RBAC（内置 admin / operator / user / auditor 四角色 + **自定义角色与操作白名单**：14 项资源域权限按组勾选授权）、前端操作入口按权限显隐、内置权限边界自动化测试；**全站安全响应头**（CSP / X-Frame-Options / nosniff / Referrer-Policy）与 gzip 传输压缩（SSE 除外，1.92.0）
- **接口限速（1.45.0）**：零依赖 IP 滑动窗口限速——写接口鉴权请求 600 次/分钟、匿名请求 60 次/分钟、匿名 Webhook 30 次/分钟（环境变量可调）
- **系统设置**：主题、界面语言（中文 / English，i18n 骨架 + 五批英文包全量覆盖（中英双语完整支持），未翻译页面自动回退中文）等偏好设置；告警静默 / 快捷引导等；设置变更审计记录旧值→新值（1.44.0）
- **日志聚合**：跨容器日志实时聚合、过滤与导出；可选持久化索引（后台每分钟增量采集，默认 7 天保留），容器重启或删除后历史日志仍可按时间 / 容器 / 关键字检索，排障刚需一步到位
- **分组侧栏导航**：全部页面按类型归类为七个可折叠分组（1Panel 风格），路由激活自动展开，菜单清爽不迷路
...
- **基线扫描（Docker Bench）**：CIS 风格一键安全体检，覆盖守护进程（live-restore / 日志轮转 / TLS）、宿主机（docker.sock 与 daemon.json 权限、2375 端口）、镜像（latest / root）与容器运行时（seccomp / pids-limit），等级化报告 + 历史比对、容器逃逸风险 Top 与扫描趋势；总览新增网络 I/O 与磁盘 I/O 速率历史曲线（10m/1h/24h/7d/30d/90d）
- **Compose 看板与滚动更新（1.34.0）**：按服务聚合项目资源（CPU / 内存 / 网络 / 磁盘 IO），服务级滚动更新（健康检查 + 失败自动回滚，1.35.0），全项目一键滚动更新、远端配置漂移检测与一键自动修复（1.40.0，本地缺失服务可勾选在远端删除，1.42.0；比对维度含卷挂载 / 网络 / healthcheck / labels，1.43.0）编排（逐服务顺序更新 + 单服务失败不中断，1.38.0）；项目镜像一键分发到远端引擎并可选**远端代理部署**（按 compose 配置在远端创建启动全部服务，1.35.0）与**远端配置漂移检测**（本地配置 vs 远端实际容器逐项比对，1.38.0）；多引擎共享内置 pull-through 镜像拉取缓存（registry:2，支持可选 htpasswd 访问认证与缓存磁盘占用查看，1.43.0）；引擎列表逐引擎连通探测并显示在线 / 离线（1.45.0）
- **资源级授权（1.38.0）**：按用户配置容器白名单（CSV + `前缀*` 通配），容器列表按名单过滤、名单外容器访问返回 403 并留痕（终端 WebSocket 同样拦截，聚合接口同步过滤可见性，1.41.0）；管理员不受限制
...
- **容器配置快照对比（1.50.0）**：容器详情一键保存配置快照（镜像 / 端口 / 环境变量 / 挂载卷 / 重启策略等），任意两份快照字段级对比，重建前后对比即可发现配置漂移
- **只读审计角色**：auditor 角色仅可查看监控 / 日志 / 审计信息，不可执行任何变更操作
- **安全加固**：2FA 双因素认证（TOTP，登录二步验证）、在线会话管理（列表 / 撤销 / 并发上限）、IP 白名单（全局 + 按用户，IPv4 CIDR）、密码策略（最小长度 / 复杂度 / 有效期过期强制改密）、登录失败锁定（账号 + IP 双维度）、安全入口（`ENTRANCE_PATH` 隐藏登录路径，未持凭证请求一律 404）；安全漏洞报告流程见 [SECURITY.md](SECURITY.md)
- **可观测性**：主机 / 容器指标 7 天原始采样 + 90 天小时级聚合长周期曲线（30d/90d 时间窗；1.36.0 起原始 / 聚合保留天数与告警记录保留条数均可在系统参数调整），总览 CPU / 内存采用「宿主机 × 容器」双维度展示并提示未配置资源限制的容器；Trivy 漏洞定时扫描计划任务，按 CVE id 差集对比、新增 Critical / High 自动推送告警，镜像详情内置扫描历史对比
...
- **Git 部署工作台**：Git 仓库绑定为部署应用，一键 clone/pull 并执行 `docker compose up -d --build`；每应用独立 Webhook token（可接 GitHub / Gitea push 事件自动部署）、加密凭据支持私有仓库、部署历史完整记录 git 与 compose 输出，失败自动推送告警；**凭据库**集中管理 Git / Registry 凭据（引用优先于内联），应用可**一键复制**做 staging / 多环境（1.85.0）
- **系统更新**：设置 → 关于**一键更新面板**——自动检测安装类型（Windows 服务 / deb / rpm），多源自动探测加速下载（用户镜像 → 公共镜像池 → 直连，1.69.0 / 1.70.0）；升级前完整备份安装目录、升级后健康检查未就绪**自动回滚**，结果留痕展示（1.73.0）；**更新提醒**——每 6 小时自动检查，新版本经通知渠道推送 + 界面右下角浮层提醒（1.74.0 / 1.74.3）
...
| `alert_rules`        | 宿主级告警规则（CPU/内存/磁盘/GPU/网络带宽/容器资源异常，含静默时段） | `server/src/alerting.ts`         |
...
| `ENTRANCE_PATH`             | 安全入口隐藏路径（设置后面板仅可经该路径访问，如 `/my-panel-9x7`） | 未启用（全网可访问登录页）  |
