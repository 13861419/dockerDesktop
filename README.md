...
- **系统更新**：设置 → 关于**一键更新面板**——自动检测安装类型（Windows 服务 / deb / rpm），多源自动探测加速下载（用户镜像 → 公共镜像池 → 直连，1.69.0 / 1.70.0）；升级前完整备份安装目录、升级后健康检查未就绪**自动回滚**，结果留痕展示（1.73.0）；**更新提醒**——每 6 小时自动检查，新版本经通知渠道推送 + 界面横幅提示（1.74.0）
