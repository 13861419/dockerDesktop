...
- **基线扫描（Docker Bench）**：CIS 风格一键安全体检，覆盖守护进程（live-restore / 日志轮转 / TLS）、宿主机（docker.sock 与 daemon.json 权限、2375 端口）、镜像（latest / root）与容器运行时（seccomp / pids-limit），等级化报告 + 历史比对；总览新增网络 I/O 与磁盘 I/O 速率历史曲线（10m/1h/24h/7d/30d/90d）
