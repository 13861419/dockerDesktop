...
- **容器管理**：列表、启停/删除/重启/克隆、镜像过滤、查看日志与详情、内置 Web 终端（xterm.js + WebSocket）、设置重启策略等；详情页含 Inspect 原始 JSON 查看（复制/下载）、容器内进程表（docker top）、按容器的操作记录留痕；**镜像自动更新**（按计划任务周期拉取镜像，有更新按原配置重建容器，健康检查未通过自动回滚，信任锁定摘要不符自动拦截）
...
- **应用商店（AppStore）**：内置应用目录，一键安装部署；Compose 套件升级前显示**当前版本 → 最新版本**对比（可升级徽标，1.46.0）
...
- **数据库可视化**：容器数据库 / Redis 的可视化查询与信息查看（只读保护）；表数据一键导出 CSV（上限 5 万行，1.46.0）
- **备份恢复**：DATA / Compose / 卷 / 站点备份恢复中心；支持将备份文件**上传到云端**（S3 / OSS / WebDAV）；数据库恢复前自动执行 SQLite 完整性校验（quick_check，1.45.0）
...
- **用户与鉴权**：登录鉴权（连续失败锁定并**持久化**，另含同 IP 滑动窗口限制防换用户名绕过，1.43.0）、登录行为审计（成功 / 失败 / 被拒含 IP 与 UA，1.44.0）、会话管理、用户增删/改密、RBAC（内置 admin / operator / user / auditor 四角色 + **自定义角色与操作白名单**：14 项资源域权限按组勾选授权）、前端操作入口按权限显隐、内置权限边界自动化测试
