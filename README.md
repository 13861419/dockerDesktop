...
- **跨引擎聚合总览**：一次展示多台 Docker 主机的资源与对象数量（单引擎离线不影响其它）；**跨引擎批量清理**（勾选多台引擎批量 prune 停止容器 / 悬空镜像 / 未使用卷 / 未使用网络，支持按年龄过滤，逐引擎汇总结果，仅管理员）
...
- **资源级授权（1.38.0）**：按用户配置容器白名单（CSV + `前缀*` 通配），容器列表按名单过滤、名单外容器访问返回 403 并留痕（终端 WebSocket 同样拦截，1.40.0）；管理员不受限制
