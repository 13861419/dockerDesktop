...
- **应用商店（AppStore）**：内置应用目录，一键安装部署；Compose 套件升级前显示**当前版本 → 最新版本**对比（可升级徽标，1.46.0）；安装前**端口冲突预检**（冲突直接拦截并列明占用来源，1.49.0）；支持 **Git 应用源**——任意 git 仓库放置 `apps.json` 清单即可批量引入应用（1.55.0）；应用可**一键导出**为 apps.json 分享片段（1.60.0）
