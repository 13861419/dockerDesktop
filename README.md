...
- **跨引擎聚合总览**：一次展示多台 Docker 主机的资源与对象数量（单引擎离线不影响其它）；**跨引擎批量清理**（勾选多台引擎批量 prune 停止容器 / 悬空镜像，逐引擎汇总结果，仅管理员）
...
- **Compose 看板与滚动更新（1.34.0）**：按服务聚合项目资源（CPU / 内存 / 网络 / 磁盘 IO），服务级滚动更新（健康检查 + 失败自动回滚，1.35.0），全项目一键滚动更新编排（逐服务顺序更新 + 单服务失败不中断，1.38.0）；项目镜像一键分发到远端引擎并可选**远端代理部署**（按 compose 配置在远端创建启动全部服务，1.35.0）与**远端配置漂移检测**（本地配置 vs 远端实际容器逐项比对，1.38.0）；多引擎共享内置 pull-through 镜像拉取缓存（registry:2）
- **资源级授权（1.38.0）**：按用户配置容器白名单（CSV + `前缀*` 通配），容器列表按名单过滤、名单外容器访问返回 403 并留痕；管理员不受限制
