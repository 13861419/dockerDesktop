...
- **Compose 看板与滚动更新（1.34.0）**：按服务聚合项目资源（CPU / 内存 / 网络 / 磁盘 IO），服务级滚动更新；项目镜像一键分发到远端引擎；多引擎共享内置 pull-through 镜像拉取缓存（registry:2）
- **供应链与容量防护（1.34.0）**：镜像信任锁定（sha256 摘要漂移检测）、磁盘写满趋势预测告警（24h 线性回归）、静默期值班渠道兜底转发
