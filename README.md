...
- **用户与鉴权**：登录鉴权- **接口限速（1.45.0）**：零依赖 IP 滑动窗口限速——写接口鉴权请求 600 次/分钟、匿名请求 60 次/分钟、匿名 Webhook 30 次/分钟（环境变量可调）
（连续失败锁定并**持久化**，另含同 IP 滑动窗口限制防换用户名绕过，1.43.0）、登录行为审计（成功 / 失败 / 被拒含 IP 与 UA，1.44.0）、会话管理、用户增删/改密、RBAC（内置 admin / operator / user / auditor 四角色 + **自定义角色与操作白名单**：14 项资源域权限按组勾选授权）、前端操作入口按权限显隐、内置权限边界自动化测试
