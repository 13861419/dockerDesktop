...
- **跨引擎聚合总览**：一次展示多台 Docker 主机的资源与对象数量（单引擎离线不影响其它）；**跨引擎批量清理**（勾选多台引擎批量 prune 停止容器 / 悬空镜像 / 未使用卷 / 未使用网络，支持按年龄过滤与 dry-run 预览，逐引擎汇总结果，仅管理员）
...
- **Compose 看板与滚动更新（1.34.0）**：按服务聚合项目资源（CPU / 内存 / 网络 / 磁盘 IO），服务级滚动更新（健康检查 + 失败自动回滚，1.35.0），全项目一键滚动更新、远端配置漂移检测与一键自动修复（1.40.0）编排（逐服务顺序更新 + 单服务失败不中断，1.38.0）；项目镜像一键分发到远端引擎并可选**远端代理部署**（按 compose 配置在远端创建启动全部服务，1.35.0）与**远端配置漂移检测**（本地配置 vs 远端实际容器逐项比对，1.38.0）；多引擎共享内置 pull-through 镜像拉取缓存（registry:2）
