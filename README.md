...
- **容器管理**：列表、启停/删除/重启/克隆、镜像过滤、查看日志与详情、内置 Web 终端（xterm.js + WebSocket）、设置重启策略等；**镜像自动更新**（按计划任务周期拉取镜像，有更新按原配置重建容器，健康检查未通过自动回滚）
...
- **供应链与容量防护（1.34.0）**：镜像信任锁定（sha256 摘要漂移检测）、磁盘写满与内存耗尽趋势预测告警（24h 线性回归）、静默期值班渠道兜底转发
...
- **可观测性**：主机 / 容器指标 7 天原始采样 + 90 天小时级聚合长周期曲线（30d/90d 时间窗），总览 CPU / 内存采用「宿主机 × 容器」双维度展示并提示未配置资源限制的容器；Trivy 漏洞定时扫描计划任务，按 CVE id 差集对比、新增 Critical / High 自动推送告警，镜像详情内置扫描历史对比
