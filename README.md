...
- **容器管理**：列表、启停/删除/重启/克隆、镜像过滤、查看日志与详情、内置 Web 终端（xterm.js + WebSocket）、设置重启策略等；详情页含 Inspect 原始 JSON 查看（复制/下载）、容器内进程表（docker top）、**运行期文件变更查看（docker diff，1.48.0）**、按容器的操作记录留痕；**镜像自动更新**（按计划任务周期拉取镜像，有更新按原配置重建容器，健康检查未通过自动回滚，信任锁定摘要不符自动拦截）
...
- **Compose 项目纳管（1.50.0）**：除面板自建项目外，自动发现宿主机上其他方式创建的 Compose 项目（手动 `docker compose up` / 第三方工具），同样支持编辑 compose 文件并 `up -d` 生效、服务状态 / 日志 / 资源查看；删除外部项目仅下线容器保留文件；编辑器支持全屏放大（1.52.0）与版本历史回退（每次保存自动记录上一版，保留最近 20 条）
...
- **用户与鉴权**：登录鉴权（连续失败锁定并**持久化**，另含同 IP 滑动窗口限制防换用户名绕过，1.43.0）、登录行为审计（成功 / 失败 / 被拒含 IP 与 UA，1.44.0）、**登录提醒**（成功 / 失败推送到通知渠道，附账号 / IP / 时间，1.49.0）、会话管理、用户增删/改密、RBAC（内置 admin / operator / user / auditor 四角色 + **自定义角色与操作白名单**：14 项资源域权限按组勾选授权）、前端操作入口按权限显隐、内置权限边界自动化测试
...
- **移动端体验（1.34.0）**：底部导航栏 + 侧栏滑动手势（≤768px）
