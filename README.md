...
- **Compose 看板与滚动更新（1.34.0）**：按服务聚合项目资源（CPU / 内存 / 网络 / 磁盘 IO），服务级滚动更新（健康检查 + 失败自动回滚，1.35.0）；项目镜像一键分发到远端引擎并可选**远端代理部署**（按 compose 配置在远端创建启动全部服务，1.35.0）；多引擎共享内置 pull-through 镜像拉取缓存（registry:2）
