...
- **Edge 节点（1.63.0 ~ 1.65.0）**：远程主机运行零依赖 agent 反向连接面板（NAT / 防火墙友好，支持 curl | sh 一键安装），面板内直接管理远端 Docker——容器全生命周期（启停 / 重启 / 删除 / 部署 / 日志）+ 远端事件并入统一事件流与自动化（跨节点自愈）；**agent 一键升级**（版本不一致时管理菜单下发升级指令，agent 自动覆盖重启，1.71.0）与**节点资源监控**（CPU / 内存使用率曲线，1.71.0）
