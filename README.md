...
- **用户与鉴权**：登录鉴权（连续失败锁定并**持久化**，另含同 IP 滑动窗口限制防换用户名绕过，1.43.0）、会话管理、用户增删/改密、RBAC（内置 admin / operator / user / auditor 四角色 + **自定义角色与操作白名单**：14 项资源域权限按组勾选授权）、前端操作入口按权限显隐、内置权限边界自动化测试
