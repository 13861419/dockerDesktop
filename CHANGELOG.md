...
- **容器日志工具栏对齐 1Panel**：日志弹窗新增「过滤」（所有 / 最近 10 分钟 / 最近 1 小时 / 最近 4 小时 / 最近 1 天，经 `since` 参数服务端过滤）、「追踪」实时轮询（原跟随刷新迁移至此并更名）、「时间戳」开关（每行前附带 Docker 时间戳）、「清空」（清空当前视图，追踪中下轮自动重填）；条数档位扩展为 100 / 200 / 500 / 1000 / 全部
