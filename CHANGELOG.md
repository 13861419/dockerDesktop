...
- **Compose 日志弹窗对齐容器日志**：补齐 过滤（所有 / 10 分钟 / 1 小时 / 4 小时 / 1 天）、条数选择（100 / 200 / 500 / 1000 / 全部）、跟随刷新（3 秒轮询 + 自动滚底）、时间戳开关、清空、复制、下载、关键字高亮搜索（含命中计数）、自动换行、到底部与行号；接口支持 `since` / `timestamps` / `tail=0`（全部）参数，「跟随刷新」移入过滤工具行
